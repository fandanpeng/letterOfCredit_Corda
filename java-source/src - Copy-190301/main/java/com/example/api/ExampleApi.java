...
public class ExampleApi {
    ...
    @Path("cash")
    @Produces(MediaType.APPLICATION_JSON)
    public Map<Currency, Amount<Currency>> getCash() {
    ...
    @Path("CPStates")
    @Produces(MediaType.APPLICATION_JSON)
    public List<StateAndRef<CPState>> GetCPStates() {
        return rpcOps.vaultQuery(CPState.class).getStates();
    }

    @PUT
    @Path("Issue-CP")
    public Response CPIssue(@QueryParam("amount") int amount, @QueryParam("strCurrency") String  strCurrency,@QueryParam("month")int month) throws InterruptedException, ExecutionException {
        if (amount <= 0) {
            return Response.status(BAD_REQUEST).entity("Query parameter 'amount' must be non-negative.\n").build();
        }
        if (month <= 0) {
            return Response.status(BAD_REQUEST).entity("Query parameter 'month' must be non-negative.\n").build();
        }
        if (strCurrency.length() != 3) {
            return Response.status(BAD_REQUEST).entity("Currency code invalid.\n").build();
        }


        try {
            final SignedTransaction signedTx = rpcOps
                    .startTrackedFlowDynamic(CPIssueFlow.class, amount, strCurrency,month)
                    .getReturnValue()
                    .get();

            final String msg = String.format("CPIssue Transaction id %s committed to ledger.\n", signedTx.getId());
            return Response.status(CREATED).entity(msg).build();
    ...
    @PUT
    @Path("Move-CP")
    public Response CPMove(@QueryParam("txHash") String  txHash, @QueryParam("index")int index, @QueryParam("newOwner") CordaX500Name newOwner ) throws InterruptedException, ExecutionException {
        if (index < 0) {
            return Response.status(BAD_REQUEST).entity("Query parameter 'index' must be non-negative.\n").build();
        }
        ...
        final Party newOwnerParty = rpcOps.wellKnownPartyFromX500Name(newOwner);
        if (newOwnerParty == null) {
            return Response.status(BAD_REQUEST).entity("Party named " + newOwner + "cannot be found.\n").build();
        }


        try {
            final SignedTransaction signedTx = rpcOps
                    .startTrackedFlowDynamic(CPMoveFlow.Initiator.class, txHash, index,newOwner)
                    .getReturnValue()
                    .get();

            final String msg = String.format("CPMove Transaction id %s committed to ledger.\n", signedTx.getId());
            return Response.status(CREATED).entity(msg).build();
    ...
    @PUT
    @Path("Redeem-CP")
    public Response CPRedeem(@QueryParam("txHash") String  txHash, @QueryParam("index")int index) throws InterruptedException, ExecutionException {
        if (index < 0) {
            return Response.status(BAD_REQUEST).entity("Query parameter 'index' must be non-negative.\n").build();
        }
        ...
        try {
            final SignedTransaction signedTx = rpcOps
                    .startTrackedFlowDynamic(CPRedeemFlow.Initiator.class, txHash, index)
                    .getReturnValue()
                    .get();

            final String msg = String.format("CPRedeem Transaction id %s committed to ledger.\n", signedTx.getId());
            return Response.status(CREATED).entity(msg).build();
